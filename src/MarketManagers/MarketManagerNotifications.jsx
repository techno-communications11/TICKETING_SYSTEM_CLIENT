...
function MarketManagerNotifications() {
  ...
  const allNotifications = useCallback(async () => {
    try {
      ...
      const filteredNoti = response.data.data
        .filter((noti) => noti.marketmanager === id)
        .map((noti) => {
          let title = '';
          let description = '';
        ...
        <Box sx={{ maxHeight: '300px', overflowY: 'auto', px: 1 }}>
          {notifications.length > 0 ? (
            notifications.map((note) => (
              <MenuItem key={note.id} onClick={() => { navigate(`/market-manager-review-ticket/${note.ticket_Id}`, handleClose()) }} sx={{ whiteSpace: 'normal' }}>
                <ListItemText
                  primary={note.title}
                  secondary={
                    <>
...
export default MarketManagerNotifications