...
// // import SearchIcon from "@mui/icons-material/Search";
// // import moment from "moment";
// // import { deleteNotificationServices, getAllNotificationsServices } from "../Services/notifications.services";
...
// //         console.log("🗑️ Delete clicked for ID:", activeId);
// //         try {
// //             const response = await deleteNotificationServices(activeId)
// //             if (response.data.statsu === 200) {
// //                 fetchAllNotifications()
// //                 handleMenuClose();
// //             };
// //         } catch (error) {
...
//     const [menuAnchor, setMenuAnchor] = useState(null);
//     const [activeId, setActiveId] = useState(null);
//     const [selectedIds, setSelectedIds] = useState([]); // ✅ multiple select
//     const isMobile = useMediaQuery("(max-width:768px)");
...
//         console.log("🗑️ Delete clicked for ID:", activeId);
//         try {
//             await deleteNotificationServices(activeId);
//             fetchAllNotifications();
//             handleMenuClose();
//         } catch (error) {
//             console.log("error", error.message);
...
//         console.log("🗑️ Bulk delete IDs:", selectedIds);

//         try {
//             // delete one by one or call backend bulk API (if exists)
//             await Promise.all(selectedIds.map((id) => deleteNotificationServices(id)));
//             setSelectedIds([]);
//             fetchAllNotifications();
//         } catch (error) {
...
//                                             }}
//                                         >
//                                             {/* Checkbox (appears when selecting) */}
//                                             {selectedIds.length > 0 && (
//                                                 <Checkbox
//                                                     checked={isChecked}
//                                                     onClick={(e) => {
...
function SuperAdminAllNotifications() {
    ...
    // ✅ Delete single (no full refresh)
    const handleDeleteSingle = async () => {
        console.log("🗑️ Delete single ID:", activeId);
        try {
            await deleteNotificationServices(activeId);
            setNotifications((prev) => prev.filter((n) => n.id !== activeId)); // remove from list
            if (selected?.id === activeId) setSelected(null);
            handleMenuClose();
        } catch (error) {
            console.log("error", error.message);
        }
                                ...
                                {filtered.map((n) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {/* 3-dot menu (only visible when not selecting) */}
                                            {selectedIds.length === 0 && (
                                                <IconButton
                                                    onClick={(e) => {
                                                        e.stopPropagation();
