...
import AddCamCastCredentials from '../SuperAdminComponent/AddCamCastCredentials';
import { getAllNewCamCredentails } from '../Services/camcredentails.services';
function SuperAdminCamCredentails() {
                            ...
                            <TableRow sx={{ backgroundColor: '#6f2da8' }}>
                                <TableCell sx={{ color: 'white' }} padding="checkbox">
                                    <Checkbox
                                        ...
                                        checked={selectedRows.length === camcastcredentail.length && camcastcredentail.length > 0}
                                        indeterminate={selectedRows.length > 0 && selectedRows.length < camcastcredentail.length}
                                    />
                                </TableCell>
                                <TableCell className='p-0' sx={{ color: 'white', width: "200px", fontSize: "14px" }}>Website</TableCell>
                            ...
                            {loader ? <TableRow><TableCell colSpan={8} align='center' sx={{ height: '500px', }}><CircularProgress /></TableCell> </TableRow> : currentRows.length > 0 ? (
                                currentRows.map((store) => (
                                    <TableRow key={store._id} hover role="checkbox" tabIndex={-1}>
                                        ...
                                        <TableCell className='p-0' sx={{ fontSize: "14px" }}>{store.website_url}</TableCell>
...
export default SuperAdminCamCredentails