...
function SeniorManagerNotifications() {
                ...
                <Box sx={{ maxHeight: '300px', overflowY: 'auto', px: 1 }}>
                    {notifications.length > 0 ? (
                        notifications.map((note) => (
                            <MenuItem key={note.id} onClick={() => { navigate(`/admin-manager-review-ticket/${note.ticket_Id}`, handleClose()) }} sx={{ whiteSpace: 'normal' }}>
                                <ListItemText
                                    primary={note.title}
                                    secondary={
                                        <>
...
export default SeniorManagerNotifications